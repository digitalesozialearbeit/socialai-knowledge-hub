...
**SOS-Unterlagen gesichtet (Nachtrag):** Kernbefunde in knowledge.md (AP 4, Stand 14.07.) destilliert: SOS hat eine formalisierte KI-Governance (Risikoassessment-Pflicht nach AI-Act-Klassen, KI-Beauftragte:r, Human-in-the-Loop, Kennzeichnungspflicht für Bilder/AV); der interne Chatbot heißt aktuell **JOSY** (früher „Hermine", GPT-basiert, intern gesichert, seit 10/2025 als begrenztes Risiko freigegeben); im gesicherten System dürfen Klarnamen einzelner Kinder zweckgebunden verarbeitet werden, Profiling/Vergleiche sind strikt verboten (AP-4-relevant: Bias trifft reale Klient:innendaten); die Pflichtschulung enthält Praxis-Prompts für Beratung/Betreuung als direkten Prompt-Set-Input. Offener-Punkt 6 (KI-Richtlinie-Deliverable) um den Kontext ergänzt, dass beide Partner schon eigene Richtlinien haben. Details (Anbieter, Budgets, Personen) bewusst nur intern. Memory `ki-tools-praxispartner` korrigiert („SOS: nichts" war veraltet).
