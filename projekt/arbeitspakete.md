# Arbeitspakete

> Stand: Kick-off (25.02.2026), inkl. dort beschlossener Änderungen gegenüber dem Originalantrag.

> [!TIP]
> Klicke auf ein Arbeitspaket, um Details zu Aufgaben, Meilensteinen, Deliverables und Stundenverteilung zu sehen.

---

<details>
<summary><strong>AP 1 -- Projektmanagement</strong> &emsp; 📅 02/2026 -- 01/2029 &emsp; 👤 Sabine (Uni Graz)</summary>

### AP 1 -- Projektmanagement

| | |
|---|---|
| **Zeitraum** | 02/2026 -- 01/2029 |
| **Lead** | Sabine (Uni Graz) |

**Aufgaben:**
- Projektkoordination
- Qualitätssicherung
- Finanzmanagement
- Risikomanagement

**Meilensteine:**
| Datum | Meilenstein |
|-------|------------|
| 25.02.2026 | Kick-off (erledigt) |
| laufend | Quartalstreffen |
| 01.10.2027 | Zwischenberichte (eCall) |
| 31.01.2029 | Abschlusstreffen |
| 31.03.2029 | Abschlussbericht |

**Deliverables:**
- Dokumentation aller Entscheidungen
- Zwischenberichte (eCall)
- Fachlicher Endbericht
- Endabrechnung

**Stundenverteilung:**
| Partner | Stunden |
|---------|---------|
| Uni Graz | 265 h |
| RI | 100 h |
| DHC | 70 h |
| SOS-KD | 65 h |
| JAW | 65 h |
| **Gesamt** | **565 h** |

</details>

---

<details>
<summary><strong>AP 2 -- Systematisches Literatur-Review</strong> &emsp; 📅 02/2026 -- 12/2026 &emsp; 👤 Susi (Uni Graz)</summary>

### AP 2 -- Systematisches Literatur-Review

| | |
|---|---|
| **Zeitraum** | 02/2026 -- 12/2026 |
| **Lead** | Susi (Uni Graz) |

> [!NOTE]
> Beim Kick-off verlängert (ursprünglich bis 07/2026). RI-Stunden werden in AP 2 verteilt. Aufteilung/Texturing zwischen RI + Uni.

**Aufgaben:**
- 2.1 PRISMA-Review (02--05/2026)
- 2.2 Verfassen (05--12/2026)
- Glossar-Erstellung

**Meilensteine:**
| Datum | Meilenstein |
|-------|------------|
| 10.06.2026 | Schriftlicher Review-Bericht |

**Deliverables:**
- Review-Bericht
- Wissenschaftliche Publikation(en)
- Glossar

**Inhaltliche Ergebnisse (erwartet):**
- Wissensstand zu Bias in generativer KI
- Ansätze zur Bias-Reduktion
- Forschungslücken
- Grundlage für alle weiteren APs

**Stundenverteilung:**
| Partner | Stunden |
|---------|---------|
| Uni Graz | 160 h |
| RI | 135 h |
| DHC | 70 h |
| SOS-KD | 10 h |
| JAW | 10 h |
| **Gesamt** | **385 h** |

</details>

---

<details>
<summary><strong>AP 3 -- Innovationsworkshops mit Führungskräften</strong> &emsp; 📅 06/2026 -- 07/2026 &emsp; 👤 Sabine (Uni Graz)</summary>

### AP 3 -- Innovationsworkshops mit Führungskräften

| | |
|---|---|
| **Zeitraum** | 06/2026 -- 07/2026 |
| **Lead** | Sabine (Uni Graz) |

> [!NOTE]
> Beim Kick-off vorgezogen (ursprünglich 07--10/2026).

**Aufgaben:**
- 3.1 Konzeption & Durchführung
- 3.2 Nachbereitung & Dokumentation
- 3.3 Wissenschaftliche Begleitung

**Format (Kick-off-Beschluss):**
- Vorab: Web/Online-Meeting für kurzen Einblick (15 Min.), FK schicken Fragen, Replay/Aufzeichnung (durchgeführt von DHC)
- 2 Halbtages-Workshops (nicht 6h am Stück), Zeitspanne ca. 1 Monat: 1. WS Juni, 2. WS Juli
- RI konzipiert & führt Halbtagesworkshop (3--4h) mit Unterstützung von Sabine & Susi
- Teilnehmer:innen: operative/individuelle FK (nicht nur Teamleitungen), inkl. Bereichsleitung
- N ~ 10--12 Führungskräfte von JAW & SOS-KD

**Offene Fragen:**
- Welche Führungskräfte genau?
- Inhaltliche Fragen klären
- Koordination mit Praxispartner:innen
- Prompts für DHC: notwendig, Connection zu FKs herstellen

**Deliverables:**
- Wissenschaftlicher Bericht
- Informationsmaterialien zu AI Act und DSGVO

**Stundenverteilung:**
| Partner | Stunden |
|---------|---------|
| Uni Graz | 430 h |
| RI | 100 h |
| DHC | 60 h |
| SOS-KD | 50 h |
| JAW | 50 h |
| **Gesamt** | **690 h** |

</details>

---

<details>
<summary><strong>AP 4 -- Experimentelle Analyse Prompt-Engineering</strong> &emsp; 📅 10/2026 -- 01/2027 &emsp; 👤 DHC</summary>

### AP 4 -- Experimentelle Analyse Prompt-Engineering

| | |
|---|---|
| **Zeitraum** | 10/2026 -- 01/2027 |
| **Lead** | DHC |

> [!NOTE]
> Kick-off-Beschluss: Unabhängig von AP 3 (nicht darauf aufbauend, kann parallel laufen).

**Aufgaben:**
- 4.1 Experimentelle Analyse von Prompt-Engineering-Strategien (DHC)

**Meilensteine:**
| Datum | Meilenstein |
|-------|------------|
| 13.11.2026 | Prompt-Set fertiggestellt |
| 29.01.2027 | Datensatz + Auswertungsbericht |

**Deliverables:**
- Standardisiertes Set ~50 Praxis-Prompts (in Abstimmung mit Praxispartner:innen)
- Datensatz der KI-Outputs
- Auswertungsbericht

**Stundenverteilung:**
| Partner | Stunden |
|---------|---------|
| Uni Graz | 150 h |
| DHC | 100 h |
| SOS-KD | 100 h |
| JAW | 100 h |
| RI | 0 h |
| **Gesamt** | **450 h** |

</details>

---

<details>
<summary><strong>AP 5 -- Konzeption Prompting-Framework</strong> &emsp; 📅 02/2027 -- 05/2027 &emsp; 👤 DHC</summary>

### AP 5 -- Konzeption Prompting-Framework

| | |
|---|---|
| **Zeitraum** | 02/2027 -- 05/2027 |
| **Lead** | DHC |

> [!NOTE]
> Kick-off-Änderung: RI erhält 10--20h für rechtliche Aspekte.

**Aufgaben:**
- 5.1 Konzeption Framework
- 5.2 Expert:innen-Feedback
- 5.3 Dokumentation

**Meilensteine:**
| Datum | Meilenstein |
|-------|------------|
| 04.04.2027 | Prompting-Module entwickelt |
| 31.05.2027 | Framework fertig |

**Deliverables:**
- Dokumentiertes Framework-Konzept
- Beispiel-Prompts
- Modularer Prompt-Baukasten
- Validierte Prompt-Patterns

**Stundenverteilung:**
| Partner | Stunden |
|---------|---------|
| Uni Graz | 110 h |
| DHC | 100 h |
| SOS-KD | 95 h |
| JAW | 80 h |
| RI | 0 h (+10--20h ergänzend) |
| **Gesamt** | **385 h (+10--20h)** |

</details>

---

<details>
<summary><strong>AP 6 -- Fragebogenerhebung</strong> &emsp; 📅 06/2027 -- 03/2028 &emsp; 👤 Susi (Uni Graz)</summary>

### AP 6 -- Fragebogenerhebung

| | |
|---|---|
| **Zeitraum** | 06/2027 -- 03/2028 |
| **Lead** | Susi (Uni Graz) |

> [!NOTE]
> Kick-off-Änderung: 2x Erhebung geplant (1. Jan 2027, 2. Mitte 2028). Zeitplan muss angepasst werden.

**Aufgaben:**
- 6.1 Fragebogenkonstruktion + Pretest
- 6.2 Durchführung
- 6.3 Auswertung
- 6.4 Ergebnisbericht

**Meilensteine:**
| Datum | Meilenstein |
|-------|------------|
| 01.10.2027 | Fragebogen finalisiert |
| 03.11.2027 | Befragung abgeschlossen |
| 10.03.2028 | Ergebnisbericht |

**Deliverables:**
- Standardisierter Fragebogen
- Anonymisierter Datensatz (SPSS/CSV)
- Auswertungsbericht mit Bedarfsanalyse (~40 S.)

**Stundenverteilung:**
| Partner | Stunden |
|---------|---------|
| Uni Graz | 720 h |
| SOS-KD | 50 h |
| JAW | 50 h |
| DHC | 0 h |
| RI | 0 h |
| **Gesamt** | **820 h** |

</details>

---

<details>
<summary><strong>AP 7 -- Co-Creation-Workshops</strong> &emsp; 📅 03/2028 -- 07/2028 &emsp; 👤 Sabine (Uni Graz)</summary>

### AP 7 -- Co-Creation-Workshops

| | |
|---|---|
| **Zeitraum** | 03/2028 -- 07/2028 |
| **Lead** | Sabine (Uni Graz) |

> [!NOTE]
> Kick-off-Änderung: SOS-KD & JAW übergreifend (wenn möglich), Gender-Aspekt beachten.

**Aufgaben:**
- 7.1 Konzeption (4 Halbtages-Workshops/Reihe)
- 7.2 Durchführung (N=16--24 Fachkräfte)
- 7.3 Nachbereitung
- 7.4 Wissenschaftliche Begleitung

**Deliverables:**
- Workshop-Konzept
- Materialien
- Ergebnisdokumentation
- Wissenschaftlicher Bericht
- Geschulte Multiplikator:innen

**Stundenverteilung:**
| Partner | Stunden |
|---------|---------|
| Uni Graz | 430 h |
| SOS-KD | 150 h |
| JAW | 150 h |
| DHC | 120 h |
| RI | 0 h |
| **Gesamt** | **850 h** |

</details>

---

<details>
<summary><strong>AP 8 -- Digitaler Orientierungsleitfaden (DOL)</strong> &emsp; 📅 01/2028 -- 01/2029 &emsp; 👤 Susi & Chris (Uni Graz / DHC)</summary>

### AP 8 -- Digitaler Orientierungsleitfaden (DOL)

| | |
|---|---|
| **Zeitraum** | 01/2028 -- 01/2029 |
| **Lead** | Susi & Chris (Uni Graz / DHC) |

**Aufgaben:**
- 8.1 Konzeption
- 8.2 Erstellung + Prompt-Repository
- 8.3 Grafische Aufbereitung
- 8.4 Veröffentlichung

**Meilensteine:**
| Datum | Meilenstein |
|-------|------------|
| 02.10.2028 | Konzept finalisiert |
| 30.01.2029 | DOL veröffentlicht |

**Deliverables:**
- DOL (frei zugänglich)
- Prompt Pattern Library
- Open-Source-Repository (GitHub)

> [!TIP]
> Erste Version des Orientierungsleitfadens bereits online: [digitalesozialearbeit.github.io/orientierungsleitfaden](https://digitalesozialearbeit.github.io/orientierungsleitfaden)

**Stundenverteilung:**
| Partner | Stunden |
|---------|---------|
| Uni Graz | 190 h |
| DHC | 170 h |
| RI | 100 h |
| JAW | 60 h |
| SOS-KD | 50 h |
| **Gesamt** | **570 h** |

</details>

---

<details>
<summary><strong>AP 9 -- Dissemination</strong> &emsp; 📅 07/2026 -- 01/2029 &emsp; 👤 RI</summary>

### AP 9 -- Dissemination

| | |
|---|---|
| **Zeitraum** | 07/2026 -- 01/2029 |
| **Lead** | RI |

**Aufgaben:**
- 9.1 Publikation Lit-Review
- 9.2 Publikation Bedarfserhebung
- 9.3 Online-Abschluss-Webinar

**Meilensteine:**
| Datum | Meilenstein |
|-------|------------|
| 26.11.2026 | Publikation Lit-Review |
| 04.09.2028 | Publikation Bedarfserhebung |
| 18.01.2029 | Abschluss-Webinar |

**Deliverables:**
- Mind. 2 peer-reviewed Publikationen
- Konferenzbeiträge
- Abschluss-Webinar
- DOL

> [!NOTE]
> Kick-off-Notiz: Publikations-Policy vereinbaren (Logos, Disclaimer, Vorab-Review). Autor:innenschaft klären.

**Stundenverteilung:**
| Partner | Stunden |
|---------|---------|
| Uni Graz | 380 h |
| DHC | 100 h |
| RI | 100 h |
| SOS-KD | 20 h |
| JAW | 20 h |
| **Gesamt** | **620 h** |

</details>

---

## Gesamtübersicht Stunden nach Partner:in

| Partner | AP 1 | AP 2 | AP 3 | AP 4 | AP 5 | AP 6 | AP 7 | AP 8 | AP 9 | **Gesamt** |
|---------|------|------|------|------|------|------|------|------|------|-----------|
| Uni Graz | 265 | 160 | 430 | 150 | 110 | 720 | 430 | 190 | 380 | **2.835** |
| DHC | 70 | 70 | 60 | 100 | 100 | 0 | 120 | 170 | 100 | **790** |
| RI | 100 | 135 | 100 | 0 | 0* | 0 | 0 | 100 | 100 | **535** |
| SOS-KD | 65 | 10 | 50 | 100 | 95 | 50 | 150 | 50 | 20 | **590** |
| JAW | 65 | 10 | 50 | 100 | 80 | 50 | 150 | 60 | 20 | **585** |

*\* RI erhält zusätzlich 10--20h in AP 5 für rechtliche Aspekte.*
